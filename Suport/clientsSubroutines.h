...
int state20(char input, int *lastState, Frame *frame, Table *clientsTable, Table *tableFilter, int *clientsFilter){
//OPÇÕES DOS CLIENTES

	//Saida da Tabela
	char *outBuffer = (char *)malloc(clientsTable->bigOfCol[4] + 2);
	bufferClean(outBuffer, clientsTable->bigOfCol[4] + 1);
	...
	case 1:
		...
		for(int i = 0; i < clientsTable->qtdLine; i++){
			tableGet(tableFilter, 0, i, outBuffer);
			if(outBuffer[0] != ';')
...
//CADASTRAR CLIENTES
	...
	if(*lastState != state){
		setupClientRegister(frame, state);
		*lastState = state;
	}
	...
	if(input == 10){
		...
		if(state == 23){//cpf
			...
			if(strLen(inputBuffer) == 10 && typeCast(inputBuffer) == 'i'){
				int value = atoi(inputBuffer);
			
				tableNewLine(clientsTable);
				tableInsert(clientsTable, 1, line, &value);//id
				return 231;	
			}
			fail = true;
		}
		else if(state == 231){//nome
			if(strLen(inputBuffer) > 2){
				tableInsert(clientsTable, 2, line-1, inputBuffer);//nome
				frameInputClean(frame);
				return 232;
			}
			fail = true;
		}
		else if(state == 232){//email
			if(strLen(inputBuffer) > 2 && typeCast(inputBuffer) == 'e'){
				tableInsert(clientsTable, 3, line -1, inputBuffer);
				frameInputClean(frame);
				tableRefresh(clientsTable);
				return 233;
			}
