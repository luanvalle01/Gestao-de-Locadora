...
void main(){
	...
	//Frames
	Frame *menuFrames = (Frame *)malloc(sizeof(Frame) * 10);
	...
	Frame *frames = (Frame *)malloc(sizeof(Frame) * 3);
	...
	while(state > -3){
		...
		//FILMES
		else if(state > 0 && state < 200 && state != 2 && state != 20  && state != 21){
			if(state == 1)
				state = state1(input, &lastState, &frames[0], height,&moviesTable[movieFilter]);
		
			else if(state == 10)//MENU DE OPÇÕES
				state = state10(input, &lastState, &menuFrames[1], &moviesTable[0], &movieFilter);
		...
		//CLIENTES
		else if(state == 2 || state > 200 && state < 300 || state == 20  || state == 21){
		
			if(state == 2)
				state = state2(input, &lastState, &frames[0], height,&clientsTable[clientFilter]);
		
			else if(state == 20)//MENU DE OPÇÕES
				state = state20(input, &lastState, &menuFrames[6], &clientsTable[0], &clientFilter);
		
			else if(state == 21)
				state = state21(input, &lastState, &clientFilter,&menuFrames[7], &clientsTable[1]);
		
			else if(state == 212)
				state = state212(input, &lastState, &clientFilter, &frames[1], &clientsTable[0], &clientsTable[1]);

			else if(state == 213)
				state = state213(input, &lastState, &clientFilter, &frames[1], &clientsTable[0], &clientsTable[1]);

			else if(state == 23 || state == 231 || state == 232)//CADASTRAR CLIENTES
				state = state23(state, input, &lastState, &frames[1], &clientsTable[0]);

			else if(state == 24 || state == 240 || state == 241 || state == 242)//ATUALIZAR CADASTRO
				state = state24(&idBuffer, state, input, &lastState, &frames[1], &clientsTable[0]);

			else if(state == 25 )//DESATIVAR CADASTRO
				state = state25(&idBuffer, state, input, &lastState, &frames[1], &clientsTable[0]);
		}
