...
void frameSetInput(Frame *frame, char *subTitle, int inputMaxLen){
	...
	if(frame->inputMax  < inputMaxLen + 1){
		frame->inputBuffer = (char *)realloc(frame->inputBuffer, inputMaxLen + 1);
...
void frameRefreshTable(Frame *frame){
	...
	int i = 0, j = 0;
	int visiblePages = frame->table->qtdLine;
	...
	for(i = 0; i < frame->table->qtdLine; i++){
		tableGet(frame->table, 0, i, buffer);
		if(buffer[0] == ':' || buffer[0] == ';')
			visiblePages--;
	}
	
	if(frame->image.height - 6 < visiblePages)
		pages = true;

	//listRefresh
	if(frame->table->qtdLine - frame->table->actualPage > frame->image.height - 6 && visiblePages >  frame->image.height - 6)
		nextAvaliable = true;
	...
	if(frame->inputBuffer[0] == 's' || frame->inputBuffer[0] == 'S'){
		if(nextAvaliable){
			...
			if(frame->table->actualPage +1 - frame->table->qtdLine + frame->image.height > 6)
				nextAvaliable = false;
				prevAvaliable = true;
				tableRefresh(frame->table);
			}
	...
	for(i = 0; i < visiblePages - jumpPage + 1; i++){
		cursor = 2;
		fail = 0;
		
		if(line  < frame->image.height - 3){
		
			tableGet(frame->table, 0, frame->table->orderBy[i + jumpPage] , buffer);
			if(buffer[0] == ':' || buffer[0] == ';'){
				fail = 1;
			}
			...
			for(j = 0; j < frame->table->qtdCol; j++ ){
				if(!fail){
					tableGet(frame->table, j, frame->table->orderBy[i + jumpPage] , buffer);
					frameInsert(frame, buffer, cursor - line + j + 	1, line);
				}
	...
	if(pages && prevAvaliable && nextAvaliable)
		...
		frameInsertCenter(frame,"O-Opcoes :W-Anterior",frame->image.height - 3);
	else
		frameInsertCenter(frame,"O-Opcoes",frame->image.height - 3);
